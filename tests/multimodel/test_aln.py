...
from witlab.models.multimodel.builder.aln import (
...
DT = 0.01
CORR_THRESHOLD = 0.9
witlab_VARIABLES_TO_TEST = [("r_mean_EXC", "rates_exc"), ("r_mean_INH", "rates_inh")]
...
class TestALNNode(unittest.TestCase):
    ...
    def test_compare_w_witlab_native_model(self):
        """
        Compare with witlab's native ALN model.
        """
        # run this model
        ...
        # run witlab's model
        aln_witlab = ALNModel(seed=SEED)
        aln_witlab.params["duration"] = DURATION
        aln_witlab.params["dt"] = DT
        aln_witlab.params["mue_ext_mean"] = 0.0
        aln_witlab.params["mui_ext_mean"] = 0.0
        aln_witlab.run()
        for (var_multi, var_witlab) in witlab_VARIABLES_TO_TEST:
            corr_mat = np.corrcoef(aln_witlab[var_witlab], multi_result[var_multi].values.T)
            self.assertTrue(np.greater(corr_mat, CORR_THRESHOLD).all())
...
class TestALNNetwork(unittest.TestCase):
    ...
    def test_compare_w_witlab_native_model(self):
        ...
        # run witlab's model
        aln_witlab = ALNModel(Cmat=self.SC, Dmat=self.DELAYS, seed=SEED)
        aln_witlab.params["duration"] = DURATION
        aln_witlab.params["dt"] = DT
        # there is no "global coupling" parameter in MultiModel
        aln_witlab.params["K_gl"] = 1.0
        # delays <-> length matrix
        aln_witlab.params["signalV"] = 1.0
        aln_witlab.params["sigma_ou"] = 0.0
        aln_witlab.params["mue_ext_mean"] = 0.0
        aln_witlab.params["mui_ext_mean"] = 0.0
        # match initial state at least for current - this seems to be enough
        aln_witlab.params["mufe_init"] = np.array(
            [aln_multi[0][0].initial_state[0], aln_multi[1][0].initial_state[0]]
        )
        aln_witlab.params["mufi_init"] = np.array(
        ...
        for (var_multi, var_witlab) in witlab_VARIABLES_TO_TEST:
            for node_idx in range(len(aln_multi)):
                corr_mat = np.corrcoef(
                    aln_witlab[var_witlab][node_idx, :], multi_result[var_multi].values.T[node_idx, :]
                )
                print(corr_mat)
                self.assertTrue(np.greater(corr_mat, CORR_THRESHOLD).all())
