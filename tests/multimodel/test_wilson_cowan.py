...
from witlab.models.multimodel.builder.base.constants import EXC
from witlab.models.multimodel.builder.wilson_cowan import (
...
from witlab.models.wc import WCModel
...
DT = 0.01
CORR_THRESHOLD = 0.75
witlab_VARIABLES_TO_TEST = [("q_mean_EXC", "exc"), ("q_mean_INH", "inh")]
...
class TestWilsonCowanNode(unittest.TestCase):
    ...
    def test_compare_w_witlab_native_model(self):
        """
        Compare with witlab's native Wilson-Cowan model.
        """
        # run this model
        ...
        # run witlab's model
        wc_witlab = WCModel(seed=SEED)
        wc_witlab.params["duration"] = DURATION
        wc_witlab.params["dt"] = DT
        # match initial state
        wc_witlab.params["exc_init"] = np.array([[wc_multi.initial_state[0]]])
        wc_witlab.params["inh_init"] = np.array([[wc_multi.initial_state[1]]])
        wc_witlab.run()
        for (var_multi, var_witlab) in witlab_VARIABLES_TO_TEST:
            corr_mat = np.corrcoef(wc_witlab[var_witlab], multi_result[var_multi].values.T)
            self.assertTrue(np.greater(corr_mat, CORR_THRESHOLD).all())
...
class TestWilsonCowanNetwork(unittest.TestCase):
    ...
    def test_compare_w_witlab_native_model(self):
        ...
        # run witlab's model
        wc_witlab = WCModel(Cmat=self.SC, Dmat=self.DELAYS, seed=SEED)
        wc_witlab.params["duration"] = DURATION
        wc_witlab.params["dt"] = DT
        # there is no "global coupling" parameter in MultiModel
        wc_witlab.params["K_gl"] = 1.0
        # delays <-> length matrix
        wc_witlab.params["signalV"] = 1.0
        wc_witlab.params["sigma_ou"] = 0.0
        # match initial state
        wc_witlab.params["exc_init"] = wc_multi.initial_state[::2][:, np.newaxis]
        wc_witlab.params["inh_init"] = wc_multi.initial_state[1::2][:, np.newaxis]
        wc_witlab.run()
        for (var_multi, var_witlab) in witlab_VARIABLES_TO_TEST:
            for node_idx in range(len(wc_multi)):
                witlab_ts = wc_witlab[var_witlab][node_idx, :]
                multi_ts = multi_result[var_multi].values.T[node_idx, :]
                if np.isnan(witlab_ts).any() or np.isnan(multi_ts).any():
                    continue
                corr_mat = np.corrcoef(witlab_ts, multi_ts)
