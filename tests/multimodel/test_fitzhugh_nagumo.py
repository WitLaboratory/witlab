...
from witlab.models.multimodel.builder.fitzhugh_nagumo import (
    FHN_DEFAULT_PARAMS,
    FitzHughNagumoMass,
...
class TestFitzHughNagumoMass(MassTestCase):
    ...
    def test_init(self):
        ...
        self.assertDictEqual({k: v for k, v in fhn.params.items() if "input" not in k}, FHN_DEFAULT_PARAMS)
...
class TestFitzHughNagumoNode(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertDictEqual({k: v for k, v in fhn[0].params.items() if "input" not in k}, FHN_DEFAULT_PARAMS)
    ...
    def test_compare_w_witlab_native_model(self):
        """
        Compare with witlab's native FitzHugh-Nagumo model.
        """
        # run this model
...
class TestFitzHughNagumoNetwork(unittest.TestCase):
    ...
    def test_compare_w_witlab_native_model(self):
        ...
        # run witlab's model
        fhn_witlab = FHNModel(Cmat=self.SC, Dmat=self.DELAYS, seed=SEED)
        fhn_witlab.params["duration"] = DURATION
        fhn_witlab.params["dt"] = DT
        # there is no "global coupling" parameter in MultiModel
        fhn_witlab.params["K_gl"] = 1.0
        # delays <-> length matrix
        fhn_witlab.params["signalV"] = 1.0
        fhn_witlab.params["coupling"] = "diffusive"
        fhn_witlab.params["sigma_ou"] = 0.0
        fhn_witlab.params["xs_init"] = fhn_multi.initial_state[::2][:, np.newaxis]
        fhn_witlab.params["ys_init"] = fhn_multi.initial_state[1::2][:, np.newaxis]
        fhn_witlab.run()
        for var in witlab_VARIABLES_TO_TEST:
            corr_mat = np.corrcoef(fhn_witlab[var], multi_result[var].values.T)
            self.assertTrue(np.greater(corr_mat, CORR_THRESHOLD).all())
