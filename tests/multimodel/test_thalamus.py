...
from witlab.models.multimodel.builder.thalamus import (
    TCR_DEFAULT_PARAMS,
    TRN_DEFAULT_PARAMS,
    ThalamicNode,
...
from witlab.models.thalamus import ThalamicMassModel
from witlab.utils.stimulus import ZeroInput

DURATION = 100.0
DT = 0.01
CORR_THRESHOLD = 0.9
witlab_VARIABLES_TO_TEST = [
    ("r_mean_EXC", "Q_t"),
    ("r_mean_INH", "Q_r"),
    ("V_EXC", "V_t"),
    ("V_INH", "V_r"),
...
class TestThalamicMass(MassTestCase):
    ...
    def test_init(self):
        ...
        self.assertDictEqual({k: v for k, v in tcr.params.items() if "input" not in k}, TCR_DEFAULT_PARAMS)
        self.assertDictEqual({k: v for k, v in trn.params.items() if "input" not in k}, TRN_DEFAULT_PARAMS)
        for thlm in [tcr, trn]:
...
class TestThalamicNode(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertDictEqual({k: v for k, v in thlm[0].params.items() if "input" not in k}, TCR_DEFAULT_PARAMS)
        self.assertDictEqual({k: v for k, v in thlm[1].params.items() if "input" not in k}, TRN_DEFAULT_PARAMS)
    ...
    def test_compare_w_witlab_native_model(self):
        """
        Compare with witlab's native thalamic model.
        """
        # run this model
        ...
        # run witlab's model
        thlm_witlab = ThalamicMassModel()
        thlm_witlab.params["duration"] = DURATION
        thlm_witlab.params["dt"] = DT
        thlm_witlab.params["V_t_init"] = np.array([-70])
        thlm_witlab.params["V_r_init"] = np.array([-70])
        thlm_witlab.run()
        for (var_multi, var_witlab) in witlab_VARIABLES_TO_TEST:
            corr_mat = np.corrcoef(thlm_witlab[var_witlab], multi_result[var_multi].values.T)
            self.assertTrue(np.greater(corr_mat, CORR_THRESHOLD).all())
